...
class mywindow(QtWidgets.QMainWindow):
    ...
    def convertToQt(self, image):
        if len(image.shape) == 2:
            ...
            p = convert_to_Qt_format.scaled(665, 500, Qt.KeepAspectRatio)
        else:
...
class ThreadFire(QThread):
    ...
    def run(self):
        ...
        def draw(bin):
            ...
            # Создание массиваа такого же типа и размера, как у оригинальных кадров
        ...
        self.color = draw(application.all.binF)
...
class ThreadSmoke(QThread):
    ...
    def run(self):
        ...
        self.color = draw(application.all.binSm)
...
class Binar:
    def smoke(self):
        ...
        mask = application.all.interSm
        # Нормализация изображения. Приведение от типа данных float64 к uint8
        ...
        if application.all.have_in:
            ...
            kT = (porog/application.all.porogSm - 1) * 100
            ...
            #print("All ok smoke")
        #porog = application.all.maxSm * 0.85
        # Бинаризация изображения
        f = open('smoke.txt','w')
        f.write(str(application.all.out_Sm) + "\n")
            ...
            while xn < (x2 - res):
                ...
                while yn < (y2 - res):
                    ...
                    if count == res * res:
                        ...
                        application._pause = False
                        application.ui.pushButton_5.setText("Play")
                    ...
                    for k in range(0, res):
                        ...
                        for l in range(0, res):
                            ...
                            if bin[y, x] == 255:
                                count = count + 1
                    if count == (res * res):
                        application.thf.color = cv2.rectangle(application.thf.color, (xn, yn), (x, y), (255, 0, 255), 9)
                        application._pause = False
        ...
        if len(ranked) > 1:
            while ranked[i] > (res * res):
                ...
                if width[j] > res and height[j] > res:
                    ...
                    if ans == 1:
                        application.thf.color = cv2.rectangle(img, (x1, y1), (x2, y2), (15, 255, 23), 3)
